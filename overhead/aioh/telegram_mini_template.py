...
# 	"""Start the bot."""
# 	chat = Chat()
# 	# Create the EventHandler and pass it your bot
#
# 	chat.updater = Updater(TOKEN)
# 	# Get the dispatcher to register handlers
#
# 	chat.dispatcher = chat.updater.dispatcher
# 	# bot listens for users leaving or logging in to the chatroom
# 	chat.dispatcher.add_handler(MessageHandler(Filters.status_update, status))
# 	# on different commands - answer in Telegram
# 	chat.dispatcher.add_handler(CommandHandler("start", start))
# 	chat.dispatcher.add_handler(CommandHandler("help", help_me))
# 	# log all errors
# 	chat.dispatcher.add_error_handler(error)
# 	# Start the Bot
# 	chat.updater.start_polling()
...
def start(bot: Bot, updater: Updater):
	"""Send a message when the command /start is issued."""
	update.message.reply_text('Hi!')


def help_me(bot: Bot, updater: Updater):
	"""Send a message when the command /help is issued."""
	update.message.reply_text('Help!')


def error(bot: Bot, updater: Updater):
...
def main_2(updater: Updater, update_id: typing.Optional[int]) -> int:
	global chatbot, id_counter, update_id_mem
	"""Start the bot."""
	chat = Chat()
	# Create the EventHandler and pass it your bot's token.
...
def main_async(updater: Updater, update_id: typing.Optional[int]) -> int:
	global chatbot, id_counter, update_id_mem
	"""Start the bot."""
	chat = Chat()
	# Create the EventHandler and pass it your bot's token.
