...
from overhead.opengloh.shaders import *
...
class ShowImage:
	def __init__(self, img_path=None, folder=None, sideways=1, upside_down=1, instant=0, ext_filter='.jpg', file_manager: FileManager = None):
...
class Canvas(app.Canvas):
	def __init__(self, canvas_size=(512, 512), tex_size=(512, 512), canvas_sca=1.0, canvas_pos=None, glsl='basic_shader_old.glsl'):
		...
		if canvas_pos is not None:
			...
			if isinstance(self.canvas_pos[0], float):
				self.canvas_pos = int(self.display_max_resolution[0] * canvas_pos[0]), int(self.display_max_resolution[1] * canvas_pos[1])
		...
		# try:
		self.shader_path = os.path.dirname(os.path.abspath(__file__)) + '\\opengloh\\shaders\\' + self.glsl
		...
		# 	# self.shader_path = f'./opengloh/shaders/basic_shader_old.glsl'
		# 	self.shader_path = f'./opengloh/shaders/basic_shader.glsl'
		# 	self.VERT_SHADER, self.FRAG_SHADER = slice_glsl(self.shader_path)
		# 	self.program = gloo.Program(self.VERT_SHADER, self.FRAG_SHADER)
		print(self.VERT_SHADER)
		print(self.FRAG_SHADER)
		
		self.program['neighbourhood'] = 1.
		self.program['show_components'] = 1
		...
		self.img_array1 = np.random.uniform(0, 1, (*self.canvas_size, 3)).astype(np.float32)
		self.img_array2 = np.random.uniform(0, 1, (*self.canvas_size, 3)).astype(np.float32)
		self.img_array3 = np.random.uniform(0, 1, (*self.canvas_size, 3)).astype(np.float32)
		self.img_array4 = np.random.uniform(0, 1, (*self.canvas_size, 3)).astype(np.float32)
		
		self.texture = gloo.Texture2D(self.img_array1, interpolation='linear', wrapping='repeat')
	...
	def update_texture(self, tex=None, use_astype=None):
		...
		# 	self.rgb_array = self.rgb_inject.astype(np.float32)
		# 	self.texture.set_data(self.self.rgb_array)
		# else:
		self.rgb_array_inject = [self.rgb_inject, self.rgb_inject, self.rgb_inject, self.rgb_inject]
		
		self.img_array1 = self.rgb_array_inject[0].astype(np.float32)
		self.texture.set_data(self.img_array1)
